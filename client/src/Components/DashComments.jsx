...
function DashComments() {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const res = await fetch(`/blog/comment/getcomments`, { credentials: 'include'});
  ...
  const handleShowMore = async () => {
    ...
    try {
      const res = await fetch(
        `/blog/comment/getcomments?startIndex=${startIndex}`,{ credentials: 'include'}
      );
      const data = await res.json();
      if (res.ok) {
  ...
  const handleDeleteComment = async () => {
    ...
    try {
      const res = await fetch(
        `/blog/comment/deleteComment/${commentIdToDelete}`,
        {
          method: 'DELETE',
          credentials: 'include',
        }
